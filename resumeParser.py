...
class Parse():
    ...
    def preprocess(self, document):
        ...
        try:
            ...
            print("Error: rid of special characters, " + str(e))
    ...
    def getExperience(self, inputString, infoDict, debug=False):
        ...
        try:
            ...
            print(traceback.format_exc())
            print("Error: getExperience, " + str(e))
        if experience:
    ...
    def getQualification(self, inputString, infoDict, D1, D2):
        ...
        nameofinstitutes = open('nameofinstitutes.txt',
        ...
        try:
            ...
            for ind, sentence in enumerate(
                    self.lines):  # find the index of the sentence where the degree is find and then analyse that sentence
            ...
            if index:  # only finds for Ca rank and CA year if it finds the word Ca in the document
                ...
                for indextocheck in index:  # checks all nearby lines where it founds the degree word.ex-'CA'
                    for i in [indextocheck,
                              indextocheck + 1]:  # checks the line with the keyword and just the next line to it
                        try:
                            try:
                                wordstr = " ".join(
                                    words[0] for words in self.lines[i])  # string of that particular line
                            except:
                                wordstr = ""
                            ...
                            for subtree in chunked_line.subtrees():
                                if subtree.label() == 'INSTI':
                                    for ind, leaves in enumerate(subtree):
                                        if leaves[0].lower() in nameofinstitutes and leaves[1] == 'NNP' and \
            ...
            print(traceback.format_exc())
            print("Error: getQualification, " + str(e))
